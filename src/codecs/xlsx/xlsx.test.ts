...
import { columnIndexToName, columnNameToIndex, XlsxCodec } from './'

const { decode, encode } = new XlsxCodec()
...
const simple = {
  file: `${__dirname}/__fixtures__/simple-dataTable.xlsx`,
  node: {
    type: 'Datatable',
    name: 'Sheet1',
    columns: [
...
const table = {
  file: `${__dirname}/__fixtures__/simple-table.xlsx`,
  node: {
    type: 'Table',
    name: 'Sheet1',
    rows: [
...
const collection: {
  ...
  file: `${__dirname}/__fixtures__/collection.xlsx`,
  node: {
    type: 'Collection',
    parts: [
      {
