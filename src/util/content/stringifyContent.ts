  ...
  if (typeof node === 'object') {
    if ('content' in node) {
      ...
      if (properties.length > 0) {
        log.warn(`Node properties ignored when stringifying: ${properties.map(prop => `\`${prop}\``).join(', ')}`)
      }
      return stringifyContent(content)
    } else {
      log.warn(`Node keys ignored when stringifying: ${Object.keys(node).map(key => `\`${key}\``).join(', ')}`)
