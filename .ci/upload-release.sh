...
#
#    upload-release.sh bin/encoda encoda x86_64-unknown-linux-gnu.tar.gz
#    upload-release.sh bin/encoda.exe encoda x86_64-pc-windows-msvc.zip
#
...
if [ "${FILE_PATH##*.}" == "${TRIPLE_FORMAT##*.}" ]; then
    ...
    if [ $ARCHIVE_FORMAT == "zip" ]; then
        if [ -x "$(command -v zip)" ]; then
            (cd $(dirname $FILE_PATH) && zip - $(basename $FILE_PATH)) > $UPLOAD_PATH
        else
...
RELEASE_ID=$(curl -s "https://api.github.com/repos/stencila/encoda/releases/tags/$TAG" | grep -m 1 "id.:" | cut -c9-16)
echo "Will upload to release $RELEASE_ID"

DOWNLOAD_NAME="$ASSET_NAME-$TAG-$TRIPLE_FORMAT"
UPLOAD_URL="https://uploads.github.com/repos/stencila/encoda/releases/$RELEASE_ID/assets?name=$DOWNLOAD_NAME"
